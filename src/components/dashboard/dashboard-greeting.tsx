...
import { Plus, Calendar, AlertTriangle, ClipboardList, ArrowRight } from "lucide-react";
...
export function DashboardGreeting({
  ...
  name: string;
  summary: { todayDeadlineCount: number; weekHearingCount: number; next7dHearingCount: number; nearTermCount: number };
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-4 lg:col-span-2">
        ...
        <div>
          ...
          <p className="mt-2 max-w-xl text-[0.875rem] leading-relaxed text-muted-foreground">
            今天有 <Num>{summary.todayDeadlineCount}</Num> 件事需处理；本周共开庭{" "}
            <Num>{summary.weekHearingCount}</Num> 场；7日内将开庭{" "}
            <Num>{summary.next7dHearingCount}</Num> 场；近期期限 <Num>{summary.nearTermCount}</Num> 项。
          </p>
        </div>
      ...
      {/* 右侧：近期日程 */}
      <div className="ll-surface min-w-0 p-3 lg:col-span-3">
        <div className="mb-2 flex items-start justify-between gap-2">
          <div>
            <h3 className="text-[13px] font-medium text-foreground">近期日程</h3>
          </div>
          <Link
            href="/schedule"
        ...
        {scheduleItems.length > 0 ? (
          <ul className="h-[150px] space-y-1.5 overflow-y-auto pr-1">
...
function ScheduleBriefItem({ item }: { item: ScheduleItem }) {
  ...
  const content = (
    <div className="flex min-w-0 items-center gap-1.5">
      ...
      <span className="inline-flex shrink-0 items-center gap-1.5 rounded-sm bg-primary/10 px-1.5 py-0.5 text-[10px] font-semibold tabular text-primary ring-1 ring-primary/15 text-nowrap">
        <span className="tabular-nums w-[5rem]">{item.date}</span>
        <span>（{item.weekday}）{item.time ?? "--:--"}</span>
      </span>
